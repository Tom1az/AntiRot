      ...
      <div className="bg-blue-600 text-white rounded-3xl p-8 shadow-md">
        ...
        <div className="flex gap-4">
          <button
            ...
            className="bg-white text-blue-700 px-6 py-2.5 font-bold rounded-lg shadow-sm text-sm hover:focus">
            Xem lộ trình tổng hợp
          </button>
          <button
            onClick={() => {
              if (highRiskEntries.length > 0) setShowRoadmapModal(highRiskEntries[0].course_module_name);
              else if (progress.length > 0) setShowRoadmapModal(progress[0].course_module_name);
            }}
            className="border border-blue-400 text-white px-6 py-2.5 font-bold rounded-lg hover:bg-blue-700 transition text-sm">
            Xem theo môn yếu nhất
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 md:p-8" onClick={onClose}>
      <div className="bg-white w-full max-w-5xl max-h-[90vh] rounded-3xl overflow-hidden flex flex-col shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b flex justify-between items-center bg-slate-50">
          <div>
            <h2 className="text-2xl font-bold text-slate-800">Lộ trình Đề xuất</h2>
            <p className="text-sm font-semibold text-slate-500">Tùy chỉnh lịch học theo mục tiêu của bạn</p>
          </div>
        ...
        <div className="bg-white border-b px-8 py-4 flex flex-wrap gap-4 items-end bg-slate-50/50">
          <div className="flex-1 min-w-[200px]">
            ...
            <select
              ...
              className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              {courses.map(c => <option key={c} value={c}>{c === 'Global' ? '🌍 Tất cả môn học (Global)' : c}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-500 mb-1">Thời gian (giờ/ngày)</label>
            <input
              type="number"
              min="1" max="12"
              value={hoursPerDay}
              onChange={e => setHoursPerDay(parseInt(e.target.value) || 2)}
              className="w-32 bg-white border border-slate-300 rounded-lg px-3 py-2 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <button
        ...
        <div className="p-8 overflow-y-auto flex-1">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="font-semibold text-slate-500">AI đang vạch ra lộ trình tốt nhất cho bạn...</p>
            </div>
          ) : graphData ? (
            <div className="space-y-8">
              <div className="bg-blue-50/50 border border-blue-100 p-6 rounded-2xl">
                <h3 className="font-bold text-blue-800 flex items-center gap-2 mb-2">
                  <Bot className="w-5 h-5" /> Nhận xét từ AI Socratic
                </h3>
                <p className="text-slate-700 font-medium leading-relaxed">
                  {graphData.ai_insight || 'Bạn đang làm tốt, hãy tiếp tục phát huy!'}
                </p>
              </div>
              ...
              {timeAllocData.length > 0 ? (
                <div className="grid grid-cols-2 gap-8 items-center bg-white border rounded-2xl p-6 shadow-sm">
                  <div>
                    <h3 className="font-bold text-slate-800 text-lg mb-4">Đề xuất Phân bổ Thời gian</h3>
                    <div className="space-y-3">
                  ...
                  <div className="h-64 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        <Tooltip formatter={(value: number) => [`${value}%`, 'Phân bổ']} />
                      </PieChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                ...
                <div className="p-6 bg-green-50 text-green-700 rounded-2xl border border-green-200 font-medium text-center">
                  Hiện tại chưa có điểm yếu nào cần phân bổ thời gian đặc biệt. Hãy học theo tiến độ bình thường!
                </div>
              )}

              {graphData.recommended_next && (
                <div>
                  <h3 className="font-bold text-slate-800 mb-3">Hành động tiếp theo:</h3>
                  <div className="bg-amber-50 border border-amber-200 p-5 rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div>
                      <span className="text-xs font-black uppercase text-amber-600 tracking-wider">Đề xuất tập trung</span>
                    ...
                    <div className="flex gap-3 w-full md:w-auto">
                      ...
                      >
                        {isTracking ? <><Check className="w-4 h-4" /> Đang theo dõi</> : <><Bell className="w-4 h-4" /> Theo dõi lộ trình</>}
                      </button>
                      <button
              ...
              {graphData.schedule && graphData.schedule.length > 0 && (
                <div>
                  <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-blue-600" /> Thời gian biểu chi tiết
                  </h3>
                      ...
                      <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                        ...
                        <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-4 rounded-2xl border border-slate-200 shadow-sm hover:border-blue-300 hover:shadow-md transition">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className="text-xs font-bold bg-slate-100 text-slate-500 px-2 py-1 rounded-md">{item.duration}</span>
                          </div>
